...
var W = class {
    ...
    update(e, t, n) {
      for (let t of e) {
        ...
        let e = (this._layers[t.id] = _(t, n));
        ((e._featureFilter = E(e.filter, `layers[${t.id}].filter`, n)),
...
var K = class {
  ...
  async parse(e, t, n, r, i) {
    ((this.data = e), (this.collisionBoxArray = new B()));
    let a = new A(Object.keys(e.layers).sort()),
      o = new j(this.tileID, this.promoteId);
    o.bucketLayerIDs = [];
    let s = {},
      l = {
        featureIndex: o,
    ...
    for (let t in u) {
      ...
      for (let e = 0; e < r.length; e++) {
        let n = r.feature(e),
          a = o.getId(n, t);
        c.push({ feature: n, id: a, index: e, sourceLayerIndex: i });
      }
      for (let e of u[t]) {
        let t = e[0];
        (t.source !== this.source &&
          ...
          !t.isHidden(this.zoom, !0) &&
            ...
            })).populate(c, l, this.tileID.canonical),
            o.bucketLayerIDs.push(e.map((e) => e.id))));
      }
    }
    let f = S(l.glyphDependencies, (e) => Object.keys(e));
    ...
    let m = Object.keys(l.iconDependencies),
      h = Promise.resolve({});
    if (m.length) {
      let e = new AbortController();
      (this.inFlightDependencies.push(e),
    ...
    let g = Object.keys(l.patternDependencies),
      _ = Promise.resolve({});
    if (g.length) {
      let e = new AbortController();
      (this.inFlightDependencies.push(e),
    ...
    let v = l.dashDependencies,
      y = Promise.resolve({});
    if (Object.keys(v).length) {
      let e = new AbortController();
      (this.inFlightDependencies.push(e),
        (y = r.sendAsync({ type: `GDA`, data: { dashes: v } }, e)));
    }
    let [b, x, C, w] = await Promise.all([p, h, _, y]),
      T = new G(b),
      E = new c(x, C);
    for (let e in s) {
      let t = s[e];
    ...
    return {
      ...
      iconMap: this.returnDependencies ? x : null,
...
function q(e, n, r) {
  let i = new t(n);
  for (let t of e) t.recalculate(i, r);
}
var J = class {
    constructor() {
  ...
  X = class {
    ...
    loadGeometry() {
      return this.pointsArray.map((e) => e.map((e) => new f(e.x, e.y)));
    }
  },
  ee = class {
    constructor(e, t, n) {
...
function ne(e, t, n) {
  ...
  for (let t = 0; t < e.length; t++) {
    let n = e.feature(t),
      l = n.loadGeometry();
    for (let e of l)
      for (let t of e) ((t.x = t.x * a - o), (t.y = t.y * a - s));
    ((l = i(l, n.type, -128, -128, r + 128, r + 128)),
      l.length !== 0 && c.push(new X(n.type, l, n.properties, n.id, r)));
  }
  return new ee(c, e.name, r);
}
var re = class {
    constructor(e, t, n) {
      ((this.actor = e),
        ...
        (this.overzoomedTileResultCache = new P(1e3)));
    }
    loadVectorTile(t, n) {
      try {
        return {
          vectorTile: t.encoding === `mlt` ? new w(n) : new O(new z(n)),
          rawData: n,
        };
      } catch (r) {
        let i = new Uint8Array(n),
          a = i[0] === 31 && i[1] === 139,
          o = `Unable to parse the tile at ${t.request.url}, `;
        throw (
          (o += a
            ? `please make sure the data is not gzipped and that you have configured the relevant header in the server`
            : `got error: ${e(r).message}`),
          Error(o)
        );
      }
    }
    async loadTile(e) {
      ...
      i.abort = a;
      try {
        let o = await m(e.request, a);
        if (e.etag && e.etag === o.etag)
          return (
            this.tileState.finishLoading(t),
            this._getEtagUnmodifiedResult(o, r)
          );
        ...
        let u = this._getExpiryData(o),
          d = this._finishRequestTiming(r);
        ((i.vectorTile = c),
          (i.etag = o.etag),
          this.tileState.markLoaded(t, i));
        let f = { rawData: l, cacheControl: u, resourceTiming: d };
        return (
          this.tileState.setParsing(t, f),
          await this._parseWorkerTile(i, e)
        );
      } catch (e) {
        throw (
          this.tileState.finishLoading(t),
    ...
    async _parseWorkerTile(e, t) {
      ...
      if (n) {
        ...
        ((r = F({ rawTileData: i.slice(0), encoding: s }, r, a, o)),
          this.tileState.removeParsing(e.uid));
      } else e.etag && (r = F(r, { etag: e.etag }));
      return r;
    }
    ...
    _getOverzoomTile(e, t) {
      ...
      let u = { vectorTile: c, rawData: T(c).buffer };
  ...
  ie = class {
    ...
    async loadTile(e) {
      let {
        ...
        c = r.width + 4,
        l = r.height + 4,
        u = D(r) ? new h({ width: c, height: l }, await N(r, -2, -2, c, l)) : r,
        d = new L(t, u, n, i, a, o, s);
  ...
  ae = class {
    ...
    loadVectorTile(e) {
      ...
      if (!i) return null;
      let a = new v(i.features, { version: 2, extent: p });
      return { vectorTile: a, rawData: T(a, s).buffer };
    }
    async loadTile(e) {
      let { uid: t } = e,
        n = new K(e);
    ...
    async _parseWorkerTile(e, t) {
      ...
      if (n) {
        let { rawData: t } = n;
        ((r = F({ rawTileData: t.slice(0), encoding: `mvt` }, r)),
          this.tileState.removeParsing(e.uid));
      }
      return r;
    }
    ...
    async loadData(e) {
      ...
      try {
        ...
        let n = {};
        return (
          e.request && (n.data = e.data),
          this._finishRequestTiming(t, e, n),
          n
        );
      } catch (e) {
    ...
    async loadAndProcessGeoJSON(e, t) {
      if ((e.request && (e.data = (await r(e.request, t)).data), e.data)) {
...
function oe(e, t) {
  let n = F(t.geojsonVtOptions || {}, { updateable: !0, clusterOptions: Z(t) });
  return new g(e, n);
}
function Z({ geojsonVtOptions: e, clusterProperties: t, source: n }) {
  ...
  for (let e of s) {
    let [a, o] = t[e],
      s = I(o, `sources.${n}.clusterProperties.${e}[1]`),
      c = I(
...
var $ = class {
  constructor(e) {
    ((this.self = e),
      ...
      (this.self.removeProtocol = n),
      (this.self.registerRTLTextPlugin = (e) => {
        b.setMethods(e);
      }),
      (this.self.makeRequest = a),
  ...
  _getWorkerSource(e, t, n) {
    if (
      ...
      switch (t) {
        case `vector`:
          this.workerSources[e][t][n] = new re(
            r,
  ...
  _getDEMWorkerSource(e, t) {
    return (
      ...
      (this.demWorkerSources[e][t] ||= new ie()),
      this.demWorkerSources[e][t]
    );
  }
};
k(self) && (self.worker = new $(self));
export { $ as default };
