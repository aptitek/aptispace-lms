...
import {
  ...
  Lr as warnOnce,
  N as ImageAtlas,
  Nn as removeProtocol,
  Rn as JSON_PREFIX,
  Tn as getArrayBuffer,
  Tt as RGBAImage,
  U as GeoJSONVT,
  Vr as EXTENT,
  Wi as Point,
  _ as createStyleLayer,
  _r as isWorker,
  c as GeoJSONWrapper,
  d as OverscaledTileID,
  dn as featureFilter,
  en as rtlWorkerPlugin,
  fn as createExpression,
  fr as isImageBitmap,
  g as Actor,
  i as MLTVectorTile,
  ir as extend,
  jn as addProtocol,
  kn as makeRequest,
  l as fromVectorTileJs,
  lt as VectorTile,
  nr as ensureError,
  o as DictionaryCoder,
  r as FeatureIndex,
  rn as register,
  t as BoundedLRUCache,
  un as groupByLayout,
  ur as getImageData,
  vt as DEMData,
  wt as AlphaImage,
...
var StyleLayerIndex = class {
  ...
  update(layerConfigs, removedIds, globalState) {
    for (const layerConfig of layerConfigs) {
      ...
      const layer = (this._layers[layerConfig.id] = createStyleLayer(
        layerConfig,
        globalState,
      ));
      layer._featureFilter = featureFilter(
        layer.filter,
        `layers[${layerConfig.id}].filter`,
        globalState,
      );
    ...
    for (const layerConfigs of groups) {
      const layers = layerConfigs.map(
        (layerConfig) => this._layers[layerConfig.id],
      );
...
var WorkerTile = class {
  ...
  async parse(
    ...
    const sourceLayerCoder = new DictionaryCoder(
      Object.keys(data.layers).sort(),
    );
    ...
    for (const sourceLayerId in layerFamilies) {
      ...
      for (const family of layerFamilies[sourceLayerId]) {
        ...
        if (layer.source !== this.source)
          warnOnce(
            `layer.source = ${layer.source} does not equal this.source = ${this.source}`,
          );
    ...
    const stacks = mapObject(options.glyphDependencies, (glyphs) =>
      Object.keys(glyphs),
    );
...
function sliceVectorTileLayer(sourceLayer, maxZoomTileID, targetTileID) {
  ...
  for (let index = 0; index < sourceLayer.length; index++) {
    ...
    featureWrappers.push(
      new VectorTileFeatureOverzoomed(
        feature.type,
        geometry,
        feature.properties,
        feature.id,
  ...
  return new VectorTileLayerOverzoomed(
    featureWrappers,
    sourceLayer.name,
    extent,
  );
...
var VectorTileWorkerSource = class {
  ...
  async loadTile(params) {
    ...
    try {
      const tileResponse = await getArrayBuffer(
        params.request,
        abortController,
      );
  ...
  async _parseWorkerTile(workerTile, params) {
    ...
    let result = await workerTile.parse(
  ...
  _getOverzoomTile(params, maxZoomVectorTile) {
    ...
    for (const sourceLayerId in layerFamilies) {
      ...
      if (!sourceLayer) continue;
      const slicedTileLayer = sliceVectorTileLayer(
        sourceLayer,
        maxZoomTileID,
        tileID.canonical,
      );
  ...
  async loadTile(params) {
    ...
    const imagePixels = isImageBitmap(rawImageData)
      ? new RGBAImage(
          ...
          await getImageData(rawImageData, -2, -2, width, height),
        )
      : rawImageData;
...
var GeoJSONWorkerSource = class {
  constructor(
    ...
    layerIndex,
    availableImages,
    createGeoJSONIndexFunc = createGeoJSONIndex,
  ) {
    this.actor = actor;
    this.layerIndex = layerIndex;
    this.availableImages = availableImages;
  ...
  _finishRequestTiming(timing, params, result) {
    ...
    result.resourceTiming = {
      [params.source]: JSON.parse(JSON.stringify(timingData)),
    };
  }
  /**
   * Implements {@link WorkerSource.reloadTile}.
  ...
  async loadAndProcessGeoJSON(params, abortController) {
    if (params.request)
      params.data = (await getJSON(params.request, abortController)).data;
    if (params.data) {
      params.data = this._filterGeoJSON(
        params.data,
        params.filter,
        params.source,
      );
    ...
    if (params.dataDiff) {
      ...
      this._geoJSONIndex.updateData(
        params.dataDiff,
        this._getFilterPredicate(params.filter, params.source),
      );
      return;
    }
    if (params.updateCluster)
      this._geoJSONIndex.updateClusterOptions(
        params.geojsonVtOptions.cluster,
        getSuperclusterOptions(params),
      );
    if (this._geoJSONIndex == null)
      throw new Error(
        `Input data given to '${params.source}' is not a valid GeoJSON object.`,
      );
  }
  /**
  ...
  _getFilterPredicate(filter, source) {
    ...
    if (compiled.result === "error")
      throw new Error(
        compiled.value.map((err) => `${err.key}: ${err.message}`).join(", "),
      );
...
function getSuperclusterOptions({
  ...
  for (const key of propertyNames) {
    ...
    const mapExpressionParsed = createExpression(
      mapExpression,
      `sources.${source}.clusterProperties.${key}[1]`,
    );
    const reduceExpressionParsed = createExpression(
      typeof operator === "string"
        ? [operator, ["accumulated"], ["get", key]]
        : operator,
      `sources.${source}.clusterProperties.${key}[0]`,
    );
...
var Worker = class {
  constructor(self) {
    ...
    this.self.registerWorkerSource = (name, WorkerSource) => {
      if (this.externalWorkerSourceTypes[name])
        throw new Error(
          `Worker source with name "${name}" already registered.`,
        );
    ...
    this.actor.registerMessageHandler("LD", (mapId, params) => {
      return this._getWorkerSource(mapId, params.type, params.source).loadData(
        params,
      );
    });
    this.actor.registerMessageHandler("LT", (mapId, params) => {
      return this._getWorkerSource(mapId, params.type, params.source).loadTile(
        params,
      );
    });
    ...
    this.actor.registerMessageHandler("AT", (mapId, params) => {
      return this._getWorkerSource(mapId, params.type, params.source).abortTile(
        params,
      );
    });
    ...
    this.actor.registerMessageHandler("UL", async (mapId, params) => {
      this._getLayerIndex(mapId).update(
        params.layers,
        params.removedIds,
        this._getGlobalState(mapId),
      );
    });
  ...
  _getWorkerSource(mapId, sourceType, sourceName) {
    ...
    if (!this.workerSources[mapId][sourceType][sourceName]) {
      ...
      switch (sourceType) {
        case "vector":
          this.workerSources[mapId][sourceType][sourceName] =
            new VectorTileWorkerSource(
              actor,
              this._getLayerIndex(mapId),
              this._getAvailableImages(mapId),
            );
          break;
        case "geojson":
          this.workerSources[mapId][sourceType][sourceName] =
        ...
        default:
          this.workerSources[mapId][sourceType][sourceName] =
            new this.externalWorkerSourceTypes[sourceType](
              actor,
              this._getLayerIndex(mapId),
              this._getAvailableImages(mapId),
            );
  ...
  _getDEMWorkerSource(mapId, sourceType) {
    ...
    this.demWorkerSources[mapId][sourceType] ||=
      new RasterDEMTileWorkerSource();
