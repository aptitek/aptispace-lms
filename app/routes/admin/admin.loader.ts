...
import { mapDbUserToStudent, type DbUserWithAffil } from "./admin.helpers";
...
export async function loadAdminDashboardData(
  ...
  const mappedUsers = dbUsers.map(mapDbUserToStudent);
  ...
  const cohorts: CohortWithInstitution[] = dbCohorts.map((c) => ({
    id: c.id,
