...
void main() {
  ...
  // Panel half-depth: w0 = 1/6
  float deltaZ = (1.0 / 6.0) * sinTheta;
  ...
  // Compute exact continuous surface normal in X-Z plane
  vec3 normal;
  if (isEven) {
    normal = normalize(vec3(-sinTheta, 0.0, cosTheta));
  } else {
    normal = normalize(vec3(sinTheta, 0.0, cosTheta));
  }
  vNormal = mix(normal, vec3(0.0, 0.0, 1.0), uProgress);

  // Perspective projection with subtle focal depth
  float camDist = 2.4;
  float zEff = camDist - z;
  float projScale = camDist / zEff;

  vec2 ndc = vec2(x * 2.0 * projScale, y * 2.0 * projScale);

  gl_Position = vec4(ndc, -z * 0.5, 1.0);
}
`;
...
void main() {
  ...
  float lightIntensity = mix(0.78 + 0.22 * diff, 1.0, uProgress);
  ...
  float mtnHighlight = smoothstep(0.08, 0.0, dMtn) * (1.0 - uProgress) * 0.18;

  // Valley grooves at hinges 2, 4 catch ambient occlusion shadow
  float dVly = min(abs(vPanelCoord - 2.0), abs(vPanelCoord - 4.0));
  float vlyShadow = smoothstep(0.10, 0.0, dVly) * (1.0 - uProgress) * 0.24;
