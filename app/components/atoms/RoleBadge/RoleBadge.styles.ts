...
import { resolveShapeStyle } from "../Avatar/shapes";
...
export function getRoleBadgeShapeName(role?: UserRole | string): string {
  const normalized = (role || "").toLowerCase().trim();
  switch (normalized) {
    case "admin":
    case "administrator":
...
const BORDER_RADIUS_MAP: Record<string, { iconOnly: string; chip: string }> = {
  admin: { iconOnly: "0px", chip: "8px 3px 8px 3px" },
  administrator: { iconOnly: "0px", chip: "8px 3px 8px 3px" },
  instructor: { iconOnly: "0px", chip: "12px 12px 4px 4px" },
  teacher: { iconOnly: "0px", chip: "12px 12px 4px 4px" },
  editingteacher: { iconOnly: "0px", chip: "12px 12px 4px 4px" },
  faculty: { iconOnly: "0px", chip: "12px 12px 4px 4px" },
};
...
interface BadgeGeometryConfig {
  borderRadius: string;
  clipPath?: string;
  hasClipPath: boolean;
}

function resolveBadgeGeometry(
  userRole: UserRole | string | undefined,
  isIconOnly: boolean,
): BadgeGeometryConfig {
  const shapeName = getRoleBadgeShapeName(userRole);
  const shapeStyle = resolveShapeStyle(shapeName);
  const fallbackRadius = getRoleBadgeBorderRadius(userRole, isIconOnly);

  if (isIconOnly && shapeStyle.clipPath) {
    return {
...
function resolveBadgeDimensions(
  sizeConfig: (typeof SIZE_MAP)["small"],
  isIconOnly: boolean,
) {
  if (isIconOnly) {
    return {
...
export const RoleBadgeRoot = styled("span", {
  ...
  const geometry = resolveBadgeGeometry(userRole, isIconOnly);
  const dimensions = resolveBadgeDimensions(sizeConfig, isIconOnly);
  ...
  return {
    ...
    boxShadow: geometry.hasClipPath
      ? "none"
      : `0 2px 8px ${alpha(paletteColor.main, 0.45)}`,
    filter: geometry.hasClipPath
      ? `drop-shadow(0 2px 4px ${alpha(paletteColor.main, 0.45)})`
      : undefined,
    ...
    "&:hover": {
      boxShadow: geometry.hasClipPath
        ? "none"
        : `0 0 12px ${alpha(paletteColor.main, 0.6)}`,
      filter: geometry.hasClipPath
        ? `drop-shadow(0 0 8px ${alpha(paletteColor.main, 0.75)})`
        : undefined,
